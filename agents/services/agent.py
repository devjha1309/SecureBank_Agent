"""Services agent with an enforced MCP tool allowlist."""

from agents.client import call_tool
from mcp_servers.contracts import TOOLS

ALLOWED_TOOLS = frozenset(name for name, spec in TOOLS.items() if spec.agent == "services")
...
async def execute(name: str, arguments: dict):
    ...
    return await call_tool("services", name, arguments)
